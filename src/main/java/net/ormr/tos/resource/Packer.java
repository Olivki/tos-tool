...
import net.ormr.tos.ipf.PkwareOutputStream;
 ...
 */
public class Packer {
    private final static Map<String, List<IpfElement>> map = new HashMap<>();
    ...
    public static void main(String... args) {
        ...
        for (Entry<String, List<IpfElement>> entry : map.entrySet())
    ...
    private static void process(String archive, List<IpfElement> elements) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(out, "rw"); FileChannel ch = raf.getChannel()) {
            int offset = 0;
            CRC32 crc = new CRC32();
            for (IpfElement element : elements) { //write compressed files
                byte[] buffer = Files.readAllBytes(element.getFile());
            ...
            for (IpfElement element : elements) { //write file table
            ...
            { //write tail
                buffer.put(IpfElement.getZipMagic());
